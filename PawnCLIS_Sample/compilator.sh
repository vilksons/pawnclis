...
while true; do
    ...
    for file in $(find "$BASHDIR" -type f -name "pawncc"); do
        BASHPAWNCC="$file"
        break
    done
    ...
    find "$BASHDIR" -type f -name "*.io*" | while read -r file; do
        AMX_O="$(dirname "$file")/$(basename "${file%.io*}.amx")"
        echo "Compiling $file -> $AMX_O"
        "$BASHPAWNCC" -i"$BASH_DIR$BASH_ALLOW_SDIR" "$file" -o"$AMX_O" -O0 -d2 || echo "Compilation failed for $file" >&2
    done
